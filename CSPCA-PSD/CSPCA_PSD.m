function [id,LP] = CSPCA_PSD(X,alpha,beta)
...
while delta > 10^-5
    if t==1
        ...
        Lp = norm(sqrt(sum(E.^2,2)),1) + alpha * norm(sqrt(sum(W.^2,2)),1) + beta * nuclear_norm;
        LP(1) = Lp;
    end
    ...
    nuclear_norm = sum(diag(W));
    Lp1 = norm(sqrt(sum(E.^2,2)),1) + alpha * norm(sqrt(sum(W.^2,2)),1) + beta *  nuclear_norm;
    delta = abs(Lp1-Lp);
    LP(t+1) = Lp1;
    Lp = Lp1;
    t = t+1;
